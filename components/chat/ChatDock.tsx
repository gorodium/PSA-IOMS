    ...
    <section className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 flex h-[min(600px,85vh)] w-[min(380px,calc(100vw-2rem))] flex-col overflow-hidden rounded-xl border border-border/50 bg-background shadow-2xl animate-in slide-in-from-bottom-10 duration-200">
      ...
      <div className="shrink-0 p-3 bg-muted/10 border-t">
        <form ref={chatFormRef} onSubmit={handleSend} className="relative rounded-lg border bg-background shadow-sm focus-within:ring-1 focus-within:ring-primary/50 transition-all flex flex-col">
          ...
          {selectedFile && (
            <div className="flex items-center justify-between gap-2 border-b bg-muted/20 px-2.5 py-1.5 text-[11px]">
              <span className="min-w-0 truncate font-medium flex items-center gap-1.5">
                <Paperclip className="h-3 w-3 text-muted-foreground" />
                {selectedFile.name}
              </span>
              <Button type="button" size="sm" variant="ghost" className="h-5 px-1.5 text-muted-foreground hover:text-foreground" onClick={() => { setSelectedFile(null); if (fileInputRef.current) fileInputRef.current.value = ""; }}>
                <X className="h-3 w-3" />
              </Button>
            </div>
          )}
